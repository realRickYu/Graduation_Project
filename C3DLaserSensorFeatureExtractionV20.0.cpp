...
vector<Mat> C3DLaserSensorFeatureExtraction::LaserSensorCylinderExtraction(Mat Img, int uplimit, int downlimint, int leftlimit, int rightlimit, int Threshold/* =150 */, int ArcLengthValue/* =50 */, Size KernelSize/* =Size */)  //返回柱面拟合所需的点云数据
{
...
Mat C3DLaserSensorFeatureExtraction::LaserSensorStudExtraction3(Mat Img, int uplimit, int downlimint, int leftlimit, int rightlimit, int Threshold/* =150 */, int ArcLengthValue/* =50 */, Size KernelSize/* =Size */)  //返回柱面拟合所需的点云数据，针对螺牙的优化，效果一般
{
...
Mat C3DLaserSensorFeatureExtraction::LaserSensorStudExtraction2(Mat Img, int uplimit, int downlimint, int leftlimit, int rightlimit, int Threshold/* =150 */, int ArcLengthValue/* =50 */, Size KernelSize/* =Size */)  //返回柱面拟合所需的点云数据
{
    Mat defaultAllPoints(1, 2, CV_64FC1, cvScalar(-1));
...
void VerticalProjection(Mat srcImage, int Threshold, Rect &ROIOutRect)//垂直积分投影
{
	Mat srcImageBin;
	int cnt=0;
...
void HorizonProjection(Mat srcImage, int Threshold, Rect &ROIOutRect)//水平积分投影
{
	Mat srcImageBin;
	int cnt = 0;
	//threshold(srcImage, srcImageBin, 120, 255, CV_THRESH_BINARY_INV);
...
Mat C3DLaserSensorFeatureExtraction::LaserSensorStudExtraction2(string strImagePath, Mat Img, int uplimit, int downlimint, int leftlimit, int rightlimit, int ArcLengthValue/* =50 */, Size KernelSize/* =Size */)  //返回柱面拟合所需的点云数据
{
	Mat defaultAllPoints(1, 2, CV_64FC1, cvScalar(-1));
...
Mat C3DLaserSensorFeatureExtraction::LaserSensorStudExtraction(Mat Img, int uplimit, int downlimint, int leftlimit, int rightlimit, int Threshold/* =150 */, int ArcLengthValue/* =50 */, Size KernelSize/* =Size */)  //返回柱面拟合所需的点云数据
{
    Mat defaultAllPoints(1, 2, CV_64FC1, cvScalar(-1));
...
vector<Mat>  C3DLaserSensorFeatureExtraction::StudFit2(vector<Mat> vecFittingPoints, Mat planePoint, double CircleMeanErrorThreshold/* =0.2*/, double CircleGrossErrorCoe/* =0.1 */, double CircleNormalErrorCoe/* =3 */)  //柱面返回中心坐标值+法线
{
	Mat defaultStudCenter(1, 3, CV_64FC1, Scalar(-1));
	...
	//Hough圆检测去除异常点，并拟合圆
	//Mat studCenter = HoughCircle(vecFittingPoints, vecXYRoted, rotInfo.XYRoted, 2.5, 2.8, 0.02, 0.02, CircleMeanErrorThreshold, CircleGrossErrorCoe, CircleNormalErrorCoe); //180525 原版
...
vector<vector<Mat>> C3DLaserSensorFeatureExtraction::distLineAndStud(vector<Mat> vecCameraCoors, double r, Mat axesMat, int bestCircle)//用于区分相机坐标系下的螺柱与平面点云
{
	...
	case 12://尝试不含第7张图 180517
			//2016.03.11 使用distLineAndStud函数区分直线和螺柱，暂时选择1.8r作为界限，并且使用StudFit2剔除只有半段的圆弧，使用HoughCircle直接提取某一整条光刀线进行圆的拟合
		{
			for (int cnt = 1; cnt != 101; cnt++)
			{
				...
				vector<Mat> studFeature = StudFit2(vecPoints[1], planeInfo.Points.t());//180525原版
			    string strStudPointPath = "StudPoint.txt";
                string strStudAxesPath = "StudAxes.txt";
			    LaserSensorIO.WriteRowFirstAPP(strStudPointPath, studFeature[0]);
			    LaserSensorIO.WriteRowFirstAPP(strStudAxesPath, studFeature[1]);
